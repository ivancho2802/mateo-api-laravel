<x-app-layout>
  ...
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
      Formulario de migracion de kobo a activiyinfo
    </h2>
  </x-slot>
      ...
      <li class="nav-item" role="presentation">
        ...
        @if(session()->has('success'))
          <div x-data="{ show: true}" x-show="show"
            class="position-fixed bg-green-500 rounded top-3 text-sm text-white py-2 px-4">
        ...
        @if(session()->has('error'))
          <div x-data="{ show: true}" x-show="show"
            class="position-fixed bg-red-500 rounded top-3 text-sm text-white py-2 px-4">
            ...
            <div class=" ">
              ...
              <div class="  lg:block">
                ...
                <div class="relative border-b border-gray-200 py-6" x-init="isOpen=true" x-data="{
                  ...
                  <h3 class="-my-3 flow-root">
                    ...
                    <button @click="isOpen = !isOpen" type="button"
                      ...
                      <span class="font-medium text-gray-900">Formulario de migracion a activity info</span>
                  ...
                  <div class="pt-6" id="filter-section-0" x-show="isOpen">
                    <div class="tab-pane fade show active" id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab"
                      ...
                      <form method="post" action="/job/deploy/migraciontoactivityinit" enctype="multipart/form-data"
                        class="row g-3 needs-validation" novalidate>
                        @csrf

                        <label for="exampleFormControlTextarea1" class="form-label">uui del formulario: </label>
                        <input type="text" placeholder="a4E3J9gkULZe5eRqQph8zh" id="id" name="id"
                          value="{{($form)['id'] ?? ''}}"
                          class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                        <div> ejemplo seria el que esta despues de forms ->
                          https://kobo2.actioncontrelafaim.org/#/forms/aeAKQW9SAMay5yLrfjEQXB/summary de este seria
                          aeAKQW9SAMay5yLrfjEQXB</div>
                ...
                <div class="relative border-b border-gray-200 py-6" x-init="isOpenA=true" x-data="{
                  ...
                  <h3 class="-my-3 flow-root">
                    <!-- Expand/collapse section button -->
                    <button @click="isOpenA = !isOpenA" type="button"
                      class="flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500"
                      aria-controls="filter-section-0" aria-expanded="false">
                      <span class="font-medium text-gray-900">Formulario para ver el proceso de migraciones</span>
                      <span class="ml-6 flex items-center">
                        <!-- Expand icon, show/hide based on section open state. -->
                        <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                          <path
                  ...
                  <div class="pt-6" id="filter-section-0" x-show="isOpenA">
                    <div class="tab-pane fade show active" id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab"
                      ...
                      <form method="post" action="/koboactivityinfo" enctype="multipart/form-data"
                        class="row g-3 needs-validation" novalidate>
                        @csrf

                        <label for="exampleFormControlTextarea1" class="form-label">Buscador: </label>
                        <input type="text" placeholder="solicitud_insumos" id="filtrar" name="filtrar" value="{{($data)['filtrar'] ?? ''}}"
                      ...
                      @if(!isset($data))
                        <div x-data="{ show: true}" x-show="show"
                          class="position-fixed bg-green-500 rounded top-3 text-sm py-2 px-4">
                          ...
                          @forelse ($data as $export)
                            <li>
                              <div class="flex items-center gap-x-6">
                                ...
                                @if((($export->download !== "")))
                                  <td class="grid_celda grid_row_activo" id="FECHA_ACTIV"
                                    data-nombre_celda="m_kobo_formularios_FECHA_ACTIV"
                                    style="width: 80px; min-width: 80px; cursor: pointer;">
                                    <a href="{{$export->download}}" target="_blank">
                                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
                                        style="width: 50px;fill: red; "><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                        <path
                                ...
                                @else
                                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
                                    style="width: 50px; "><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                    <path
                                ...
                                <div>
                                  <h3 class="text-base font-semibold leading-7 tracking-tight text-gray-900">Nombre Clave:
                                    {{$export->name_key}}</h3>
                                  <h3 class="text-base font-semibold leading-7 tracking-tight text-gray-900">
                                    ...
                                    @else
                                      <form method="get" action="/koboactivityinfo" enctype="multipart/form-data"
                                        class="row g-3 needs-validation" novalidate>
                                        @csrf
                                        <input type="hidden" placeholder="solicitud_insumos" id="formid" name="formid"
                                          value="{{$formid ?? ''}}"
                                          class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                                  ...
                                  <p class="text-sm font-semibold leading-6 text-indigo-600">Export. Procesadas:
                                    {{$export->exportaciones_procesadas}}</p>
                                  ...
                                  <p class="text-sm font-semibold leading-6 text-indigo-600">
                                    ...
                                    @if($export->exportaciones_fallidos > 0)
                                      <form method="get" action="/koboapdfrepair" enctype="multipart/form-data"
                                        class="row g-3 needs-validation" novalidate>
                                        @csrf
                                  ...
                                  <p class="text-sm font-semibold leading-6 text-indigo-600">Export. proceso:
                                    {{$export->trabajos_en_proceso}}</p>
                                </div>
                              </div>
                            </li>
                          @empty

...
select * from migrate_customs where "table"='M_MQR' and created_at >= '2025-06-10'  order by created_at ASC -- limit 10 id_user_mireview like '%1291%'
...
select * from "M_LPA_EMERGENCIAS" limit 10
select "FASE_ATENCION" from "M_LPAS" GROUP BY "FASE_ATENCION" limit 10
 
SELECT COUNT("COD_EMERGENCIAS") 
FROM 
"M_LPAS", "M_LPA_EMERGENCIAS" 
where  
"M_LPAS"."FK_LPA_EMERGENCIA" = "M_LPA_EMERGENCIAS"."ID" AND
"M_LPAS"."FASE_ATENCION" != 'Fase III-RecuperaciÃ³n temprana' 
GROUP BY "COD_EMERGENCIAS"; -- FILTRAR POR RR
-- actualizar papayal codigo a _1
-- litoral del san juan esta mal debe ser el codigo correcto
-- si eres parte del consorcio y quieres acceder aqui click aqui
-- QUIENES SOMOS DEJAR EN PRIMERA PERSONA TODO
-- tarjetas de historias para mejorar diseño
-- ponerle a las tarjetas que las categorias salgan en las tarjetas
-- historias: buscan donde diga confinamiento desplazamiento masivo y ponerla como otro filtro
-- todas las ern don rr si hay rt esta mal revisar las publicaciones  lari y bomo
-- menos visible el codigo y poner departamento y municipio en las tarjetas de 
-- que las fichas de cieere lo digan como lo dice la ern
